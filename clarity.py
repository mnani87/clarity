...
class ClarityNotes(QMainWindow):
    ...
    def save_file(self):
        # If the file is opened from disk and has a known path, save to that file
        if self.current_file_path and self.is_modified:
            try:
                if self.current_file_path.endswith('.odt'):
                    self.save_as_odt(self.current_file_path)
                elif self.current_file_path.endswith('.odf'):
    ...
    def open_file(self):
        ...
        file_name, _ = QFileDialog.getOpenFileName(self, "Open File", "", 
                                                   "Text Files (*.txt);;OpenDocument Text (*.odt);;OpenDocument Format (*.odf);;Python Files (*.py);;JavaScript Files (*.js);;HTML Files (*.html);;All Files (*)")
        if not file_name:
            return  # Return if no file is selected
        
        # Determine the file type and read the content
        try:
            if file_name.endswith(('.txt', '.py', '.js', '.html')):
            ...
            elif file_name.endswith('.odt') or file_name.endswith('.odf'):
                # Handle opening .odt and .odf files (requires odfpy)
